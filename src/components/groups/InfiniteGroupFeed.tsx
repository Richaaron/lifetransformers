...
interface InfiniteGroupFeedProps {
  groupId: string
  initialPosts: any[]
...
export function InfiniteGroupFeed({ groupId, initialPosts, initialReactionMap, currentUserId, hasMoreInitial }: InfiniteGroupFeedProps) {
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/feed/group/${groupId}?page=${page}`)
  ...
  }, [page, hasMore, isLoading, groupId])
  ...
  if (posts.length === 0 && !isLoading) {
    return (
      <div className="glass-strong border border-surface-800 rounded-2xl p-12 text-center">
        ...
        <p className="text-surface-400">Be the first to post in this group!</p>
      </div>
    )
  }
